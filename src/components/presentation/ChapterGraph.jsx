        ...
        item.concepts.forEach((concept, cIdx) => {
          ...
          if (concept.formulas) {
            if (Array.isArray(concept.formulas)) {
              formulaList = concept.formulas.filter(
          ...
          if (formulaList.length > 0) {
            console.log(
              `    ${formulaList.length}개의 Formula 노드 생성 시작...`,
            );
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{
                backgroundColor: NODE_COLORS.topic.bg,
                border: `2px solid ${NODE_COLORS.topic.border}`,
              }}
            />
            <span className="text-xs font-semibold text-slate-700">
              Topic ({nodesByType.topic})
            </span>
          </div>
          <div className="flex items-center gap-2">
            <div
              className="w-4 h-4"
              style={{
                backgroundColor: NODE_COLORS.concept.bg,
                border: `2px solid ${NODE_COLORS.concept.border}`,
              }}
            />
            <span className="text-xs font-semibold text-slate-700">
              Concept ({nodesByType.concept})
            </span>
          </div>
          <div className="flex items-center gap-2">
            <div
              className="w-4 h-4 rounded-full"
              style={{
                backgroundColor: NODE_COLORS.formula.bg,
                border: `2px solid ${NODE_COLORS.formula.border}`,
              }}
            />
            <span className="text-xs font-semibold text-slate-700">
              Formula ({nodesByType.formula})
            </span>
          </div>
        </div>
              ...
              {selectedNode.formulas && selectedNode.formulas.length > 0 && (
                <div className="mb-6">
                  <div className="text-sm font-bold text-gray-500 mb-3">
                    {selectedNode.type === "formula"
                      ? "수식"
                      : `하위 수식 (${selectedNode.formulas.length}개)`}
                  </div>
