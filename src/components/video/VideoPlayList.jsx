...
// 💡 [주의] 위젯 컴포넌트 임포트 경로를 실제 파일 위치에 맞게 수정해주세요!
...
// 💡 DB의 widget_type과 실제 리액트 컴포넌트를 연결해주는 창고입니다.
const WIDGET_MAP = {
  // "trig_circle": InteractiveUnitCircle, // 주석을 풀고 실제 컴포넌트와 연결하세요.
};
...
const DetailModal = ({ selectedVideo, onClose, onRead }) => {
  ...
  // ⭐ 현재 선택된 비디오의 widgetType에 맞는 컴포넌트를 창고(WIDGET_MAP)에서 꺼내옵니다.
    ...
    const fetchVideos = async () => {
      try {
        ...
        if (data && data.length > 0) {
          // 백엔드 데이터(Neo4j)를 프론트엔드 형식에 맞게 매핑
          const mappedData = data.map((v, index) => ({
            id: v.id || index, // 백엔드에서 전달된 lecture_id 사용
            title: v.title || "제목 없음",
            videoUrls: v.video_url ? [v.video_url] : [],
            subject: v.subject || "영상 강의",
            description: v.message || "강의 설명이 없습니다.",
        ...
        console.warn("⚠️ 데이터를 불러오지 못했습니다:", error.message);
      } finally {
        setIsLoading(false);
      }
    };
