...
const ChapterGraphPage = () => {
  ...
  const fetchChapterData = async () => {
    ...
    try {
      ...
      console.log('📦 전체 데이터:', {
        ...
        sample: Array.isArray(allData) && allData.length > 0 ? allData[0] : null,
      });
      ...
      console.log(`✅ 필터링 완료: ${filteredData.length}개 토픽 (원본: ${allData.length})`);
  ...
  const goToPrevious = () => {
    if (currentIndex > 0) {
      ...
      navigate(`/presentation/${subject}/${prevChapter.code}`);
    }
  };
  ...
  const goToNext = () => {
    if (currentIndex < chapterList.length - 1) {
      ...
      navigate(`/presentation/${subject}/${nextChapter.code}`);
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="text-2xl font-black text-slate-800">
                  {currentSubject?.title} - {chapterList[currentIndex]?.title || '로딩 중...'}
                </h1>
              </div>
            </div>
