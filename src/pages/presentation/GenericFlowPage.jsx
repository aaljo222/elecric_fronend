...
import "katex/dist/katex.min.css"; // LaTeX 스타일 필수
import {
  BookOpen,
  ChevronRight,
  Info,
...
// ✅ [수정] 색상 팔레트 (가독성 극대화: 흰색 배경엔 검은 글씨)
const NODE_STYLES = {
  chapter: {
    bg: "#1e3a8a",
    color: "#ffffff",
...
export default function GenericFlowPage({ subject: propSubject }) {
    ...
    () => [
      {
        selector: "node",
        style: {
          // ✅ 라벨 필터링: 노드 화면에서는 깨지는 LaTeX 코드 대신 깔끔한 텍스트 표시
          label: (ele) => {
            const lbl = ele.data("label") || "";
            // $수식$ 형태가 있으면 제거하거나 단순화 (화면 표시용)
            return lbl.replace(/\$[^$]*\$/g, "").trim() || lbl;
          },
          "text-valign": "center",
          "text-halign": "center",
          "font-family": "Pretendard, sans-serif",
          "font-weight": 800, // 폰트 더 굵게
          "text-wrap": "wrap",
          "text-max-width": "120px",
          "z-index": 10,
          "overlay-opacity": 0, // 클릭 시 파란 박스 제거
        },
      },
      // 챕터
      {
    ...
    const fetchChapters = async () => {
      try {
        const res = await axios.get(
          `${API_BASE}/api/presentation/chapters/list`,
          {
  ...
  const handleChapterSelect = async (chapter) => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/api/presentation/children`, {
      ...
      setElements([rootNode, ...baseNodes, ...edges]);
  ...
  const handleNodeClick = useCallback(async (nodeData) => {
    ...
    try {
      ...
      if (nodes.length === 0) {
        ...
        // 원형 배치 좌표 계산
        const positionedChildren = calculateCircularPositions(parentPos, nodes);
        ...
        setElements((prev) => {
          ...
          const newEdges = edges.filter((e) => !existIds.has(e.data.id));
  ...
  const calculateCircularPositions = (centerPos, newNodes) => {
    ...
    if (count === 0) return [];
    const radius = Math.max(180, count * 35); // 반지름 넉넉하게
  ...
  const openDetailModal = (cyNode, nodeData) => {
    console.log("openDetailModal", cyNode, nodeData);
    let relatedLatex = [];
    if (nodeData.type === "concept") {
      // cyNode.neighborhood('node[type="formula"]').forEach((n) => {
      //   if (n.data("latex")) relatedLatex.push(n.data("latex"));
      // });
    }
      ...
      {isModalOpen && selectedNode && (
        ...
        >
          <div
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation(); // 모달 내부 클릭 시엔 안 닫힘
            }}
          >
