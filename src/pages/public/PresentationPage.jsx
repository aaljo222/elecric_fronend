                ...
                <div className="flex-1 space-y-4">
                  ...
                  {concept.definition && (
                    <div className="mt-4 p-4 bg-slate-50 rounded-xl border-l-4 border-slate-400 ml-5">
                      <span className="text-sm font-bold text-slate-500 uppercase block mb-1">Definition</span>
                      <p className="text-lg text-slate-700">{concept.definition}</p>
                    </div>
                  )}
                </div>
                    ...
                    <div className="bg-amber-50 rounded-2xl border border-amber-100 p-6 shadow-sm group-hover:border-amber-300 transition-colors">
                      ...
                      <div className="space-y-6">
                        {concept.formulas.map((f, fIdx) => (
                          <div key={fIdx} className="overflow-x-auto py-2 flex justify-center">
...
export default function PresentationPage() {
  ...
  const fetchNext = useCallback(async () => {
    ...
    try {
      // API 호출
      const res = await fetch(`${API_BASE}/api/presentation/chapters?after=${lastOrder}&limit=3`);
      const data = await res.json();

      if (Array.isArray(data) && data.length > 0) {
        setItems((prev) => {
            // 중복 제거 (혹시 모를 중복 방지)
            const newItems = data.filter(d => !prev.some(p => p.code === d.code));
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-12 px-4 md:px-8">
      <div className="w-full mx-auto max-w-7xl"> {/* 너무 넓어지지 않게 max-w 설정 권장 */}
        ...
        >
          ...
          {!hasMore && !loading && (
            <span className="opacity-50 text-base">모든 내용을 불러왔습니다.</span>
          )}
        </div>
      </div>
    </div>
